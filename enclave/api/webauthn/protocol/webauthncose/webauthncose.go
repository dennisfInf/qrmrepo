...
func (k *EC2PublicKeyData) Verify(data []byte, sig []byte) (bool, error) {
	log.Print("Verify Elliptic Curve Public Key Signature")
	...
	e := &ECDSASignature{}
	// Hashing is done inside the enclave
	...
	log.Print("Using the enclave to verify the signature")
	log.Printf("XCord length: %d\nYCord length: %d\n", len(k.XCoord), len(k.YCoord))
	log.Printf("msg length: %d\nsig length: %d\n", len(data), len(sig))
	log.Printf("XCord as hex: %s\n YCord as hex: %s\n", hex.EncodeToString(k.XCoord), hex.EncodeToString(k.YCoord))
	log.Printf("XCord as int: %d\n", pubkey.X)
	log.Printf("msg as hex: %s\n", hex.EncodeToString(data))

	// Use the enclave to verify the signature
	return C.host_verify_secp256r1_sig((*C.uchar)(C.CBytes(data)), (C.uint)(len(data)), (*C.uchar)(C.CBytes(sig)), (C.uint)(len(sig))) == 1, nil
